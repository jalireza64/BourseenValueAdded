 using System;
using System.Text;
using System.Web.Mvc;
using System.Linq;
using App_Resources;
using EmployeeRequest.Infrastracture.ActionFilters;
using EmployeeRequest.Infrastracture.ActionResults;
using EmployeeRequest.Infrastracture.BaseClasses;
using EmployeeRequest.Infrastracture.Enums;
using EmployeeRequest.Infrastracture.Exceptions;

using System.Configuration;
using System.ComponentModel.DataAnnotations;
using EmployeeRequest.ViewModel;
using EmployeeRequest.BaseClasses;
using System.Data.Entity;
using EmployeeRequest.SpClasses;
using System.Collections.Generic;
using DeviceId;
using System.Net.Http;
using EmployeeRequest.Infrastracture.Config;
using EmployeeRequest.Repository;
using EmployeeRequest.Infrastracture.Helpers;

namespace EmployeeRequest.Controllers
{
    public partial class AccountController : BaseController
    {

        [HttpPost]
        public virtual ActionResult GetAllCompany()
        {
            var companies = CompanyRepository.GetAllCompany().Select(t => new { t.company_name, t.comp_id });
            return Json(companies);
        }

        [HttpPost]
        public virtual ActionResult GetCompanyLoginMessage(decimal compId)
        {
            var loginMessage = CompanyRepository.GetCompany(compId).Select(t => new { t.login_message}).FirstOrDefault();
            return Json(loginMessage);
        }

        [HttpPost]
        public virtual ActionResult GetCompanyLoginType(decimal compId)
        {
            var loginType = CompanyRepository.GetCompany(compId).Select(t => new { t.login_type }).FirstOrDefault();
            return Json(loginType);
        }

        [HttpPost]
        [ValidateModelFilter]
        public virtual ActionResult Login(LoginModel loginModel)
        {
            var username = AesEncryptDecryptor.Decrypt(loginModel.Username);
            var password = AesEncryptDecryptor.Decrypt(loginModel.Password);
            var captchaText = AesEncryptDecryptor.Decrypt(loginModel.CaptchaText);
            var compId = CompanyRepository.isOneCompanyOnly() ? CompanyRepository.GetAllCompany().FirstOrDefault().comp_id : Convert.ToDecimal(AesEncryptDecryptor.Decrypt(loginModel.CompId));

            var loginType = CompanyRepository.GetCompany(compId).Select(t => new { t.login_type }).FirstOrDefault().login_type;

            if (string.Equals(captchaText, Session["CaptchaString"].ToString(), StringComparison.CurrentCultureIgnoreCase))
            {
                shareholder shareholderObject = new shareholder();
                using (var context = new capitalEntities())
                {
                    var currentDate = DateTimeHelper.ToPersianDate(DateTime.Now).Replace("/","");
                    var loginQuery = context.shareholders;

                    if(loginType == "1")
                    {
                        shareholderObject = loginQuery.Where(t => t.bbs_code.Replace("ك", "ک").Replace("ي", "ی") == username && t.password == password && t.comp_id == compId && currentDate == t.shr_meet_date).ToList().FirstOrDefault();
                    }
                    else
                    {
                        shareholderObject = loginQuery.Where(t => t.shrh_code == username && t.password == password && t.comp_id == compId && currentDate == t.shr_meet_date).ToList().FirstOrDefault();
                    }


                    if (shareholderObject == null)
                        return Json(ResponseType.Failed, MessagesLibrary.InvalidUsernameOrPassword);

                    var LoginResult = new LoginResultModel
                    {
                        Name = shareholderObject.name,
                        Family = shareholderObject.surname,
                        BBSCode = shareholderObject.bbs_code,
                        CompId = shareholderObject.comp_id,
                        ShrhCode = shareholderObject.shrh_code,
                        UserType = "1"
                    };
                    Session.Add("LoginResult", LoginResult);
                    
                }

                return Json(ResponseType.Ok, "");
            }
            else
            {
                return Json(ResponseType.Warning, string.Format(MessagesLibrary.SelectedValueIsInvalid, CaptionsLibrary.CaptchaText));
            }
        }

        [HttpPost]
        [ValidateModelFilter]
        public virtual ActionResult ManagementLogin(LoginModel loginModel)
        {
            var username = AesEncryptDecryptor.Decrypt(loginModel.Username);
            var password = loginModel.Password;
            var captchaText = AesEncryptDecryptor.Decrypt(loginModel.CaptchaText);
            var compId = CompanyRepository.isOneCompanyOnly() ? CompanyRepository.GetAllCompany().FirstOrDefault().comp_id : Convert.ToDecimal(AesEncryptDecryptor.Decrypt(loginModel.CompId));

            if (string.Equals(captchaText, Session["CaptchaString"].ToString(), StringComparison.CurrentCultureIgnoreCase))
            {
                management_user managementUserObject = new management_user();
                using (var context = new capitalEntities())
                {
                    var loginQuery = context.management_user;
                    managementUserObject = loginQuery.Where(t => t.user_id == username && t.password == password  && t.comp_id == compId).ToList().FirstOrDefault();

                    if (managementUserObject == null)
                        return Json(ResponseType.Failed, MessagesLibrary.InvalidUsernameOrPassword);

                    var LoginResult = new LoginResultModel
                    {
                        Name = managementUserObject.name,
                        Family = managementUserObject.surname,
                        CompId = managementUserObject.comp_id,
                        ManagementUserId = managementUserObject.user_id,
                        UserType = "2"
                    };
                    Session.Add("LoginResult", LoginResult);
                }

                return Json(ResponseType.Ok, "");
            }
            else
            {
                return Json(ResponseType.Warning, string.Format(MessagesLibrary.SelectedValueIsInvalid, CaptionsLibrary.CaptchaText));
            }
        }

        [HttpPost]
        public virtual ActionResult Logout()
        {
            Session["LoginResult"] = null;
            return Json(ResponseType.Ok, "");
        }

        public virtual CaptchaImageResult ShowCaptchaImage()
        {
            return new CaptchaImageResult();
        }

        public virtual ActionResult SessionExpired()
        {
            var sessionExpired = Session["LoginResult"];
            return Json(ResponseType.Ok, "", sessionExpired);
        }

        #region private methods

        [HttpGet]
        public virtual ActionResult GetPassRegexProps()
        {
            var regexPattern = ConfigurationManager.AppSettings["PassRegexPattern"];
            var regexError = ConfigurationManager.AppSettings["PassRegexError"];
            return Json(new
            {
                RegexPattern = regexPattern,
                RegexError = regexError,
            }, JsonRequestBehavior.AllowGet);
        }

        #endregion

        [HttpPost]
        public virtual ActionResult GetCompanyLogo()
        {
            var data = "data:image/png;base64,";
            return Json(data);
        }

        [HttpPost]
        public virtual ActionResult ValidateContract()
        {
            string deviceId = new DeviceIdBuilder()
                .AddMachineName()
                .AddProcessorId()
                .AddMotherboardSerialNumber()
                .AddSystemDriveSerialNumber()
                .ToString();

            var cstCode = AppSetting.GetCstCode();
            var lockModel = ValidateDeviceId(deviceId, cstCode);

            var isValidDate = Convert.ToDecimal(lockModel.ValidDate) - Convert.ToDecimal(DateTimeHelper.ToPersianDate(DateTime.Now).Replace("/", "")) < 0 ? false : true;
            var isValidContract = lockModel.IsValidContract && isValidDate;
            var customerName = lockModel.CustomerName;
            var isOneCompanyOnly = CompanyRepository.isOneCompanyOnly();
            var result = new
            {
                deviceId,
                isValidContract,
                customerName,
                isOneCompanyOnly
            };

            return Json(result);
        }

        public static LockModel ValidateDeviceId(string deviceId, string cstCode)
        {
            LockModel lockModel = new LockModel();

            using (var context = new capitalEntities())
            {
                var lockObject = context.Locks.Where(t => t.Id == context.Locks.Max(x => x.Id)).FirstOrDefault();
                if (lockObject == null)
                {
                    GetFromLock(deviceId, cstCode, lockModel, context);
                }
                else
                {
                    if (AesEncryptDecryptor.Decrypt(lockObject.DeviceId) != deviceId)
                    {
                        GetFromLock(deviceId, cstCode, lockModel, context);
                    }
                    else
                    {
                        lockModel.CustomerName = AesEncryptDecryptor.Decrypt(lockObject.CustomerName);
                        lockModel.DeviceId = AesEncryptDecryptor.Decrypt(lockObject.DeviceId);
                        lockModel.ValidDate = AesEncryptDecryptor.Decrypt(lockObject.ValidDate);
                        lockModel.IsValidContract = true;
                    }
                }
            }

            return lockModel;
        }

        private static void GetFromLock(string deviceId, string cstCode, LockModel lockModel, capitalEntities context)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri("http://lock.raadeen.ir/api/");

                //Called Member default GET All records  
                //GetAsync to send a GET request   
                // PutAsync to send a PUT request  
                var responseTask = client.GetAsync("lock?" + "DeviceId=" + deviceId + "&CstCode=" + cstCode + "&PrdId=5");
                responseTask.Wait();

                //To store result of web api response.   
                var result = responseTask.Result;

                //If success received   
                if (result.IsSuccessStatusCode)
                {
                    var readTask = result.Content.ReadAsAsync<string>();
                    readTask.Wait();

                    if (readTask.Result == "False")
                    {
                        lockModel.IsValidContract = false;
                        lockModel.CustomerName = CaptionsLibrary.TrialVersion;
                        lockModel.DeviceId = deviceId;
                    }
                    else
                    {
                        var outPut = readTask.Result.Split('|');
                        lockModel.IsValidContract = true;
                        lockModel.CustomerName = outPut[0];
                        lockModel.DeviceId = deviceId;
                        lockModel.ValidDate = outPut[1];

                        var lockObj = new Lock
                        {
                            CustomerName = AesEncryptDecryptor.Encrypt(lockModel.CustomerName),
                            DeviceId = AesEncryptDecryptor.Encrypt(lockModel.DeviceId),
                            ValidDate = AesEncryptDecryptor.Encrypt(lockModel.ValidDate)
                        };

                        var lockDbSet = context.Set<Lock>();
                        lockDbSet.Add(lockObj);
                        context.SaveChanges();
                    }

                }
                else
                {
                    //Error response received   
                    lockModel.IsValidContract = false;
                    lockModel.CustomerName = MessagesLibrary.PleaseActivateTheSoftware;
                }
            }
        }

        [HttpPost]
        public virtual ActionResult ShowCaptchaImageInByte()
        {
            var captchaImageResult = new CaptchaImageResult();
            var base64Image = captchaImageResult.GetCaptchaImageAzByte(ControllerContext);

            return Json(ResponseType.Ok, String.Empty, base64Image);
        }

        [HttpPost]
        public virtual ActionResult GetAllManagementUser()
        {
            var loginResult = (LoginResultModel)Session["LoginResult"];
            var result = ManagementUserRepository.GetAllManagementUser(loginResult.CompId);
            if (result == null)
                return Json(ResponseType.Failed, MessagesLibrary.OperationFailed);
            return Json(result.Select(t=> new {
                t.name,
                t.surname,
                t.mobile,
                t.user_id
            }));
        }

        [HttpPost]
        public virtual ActionResult addManagementUser(string name, string surname, string userId, string password, string mobile)
        {
            var loginResult = (LoginResultModel)Session["LoginResult"];
            var managementUser = new management_user
            {
                name = name,
                surname = surname,
                user_id = userId,
                password = password,
                mobile = mobile,
                comp_id= loginResult.CompId
            };
            var result = ManagementUserRepository.AddManagementUser(managementUser);
            if (!result)
                return Json(ResponseType.Failed, MessagesLibrary.OperationFailed);
            return Json(ResponseType.Ok, MessagesLibrary.OperationSuccessed);
        }

        [HttpPost]
        public virtual ActionResult RemoveManagementUser(string userId)
        {
            var loginResult = (LoginResultModel)Session["LoginResult"];
            var managementUser = new management_user
            {
                user_id = userId,
                comp_id = loginResult.CompId
            };
            var result = ManagementUserRepository.RemoveManagementUser(managementUser);
            if (!result)
                return Json(ResponseType.Failed, MessagesLibrary.OperationFailed);
            return Json(ResponseType.Ok, MessagesLibrary.OperationSuccessed);
        }
    }
}