        ...
        <div class="bs-docs-example k-content flex" v-if="kendoMessages">
            ...
            <kendo-grid :data-source="reportDataSource"
                ...
                <kendo-grid-column :field="'vote'"
                                   ...
                                   :footer-template="`#: data.vote ? kendo.toString(data.vote.sum, 'n0') : 0 #`"></kendo-grid-column>

                <kendo-grid-column :field="'vote_validity'"
                                   :width="100"
                                   :template="voteValidityTemplate()"
                                   :title="$CaptionsLibrary.get('Validity')"
                                   :footer-template="`#: data.vote_validity ? kendo.toString(data.vote_validity.count, 'n0') : 0 #`"></kendo-grid-column>
            </kendo-grid>
        </div>
    </div>
</template>

<script lang="ts" src="./rptMeetingVote.ts">
</script>
